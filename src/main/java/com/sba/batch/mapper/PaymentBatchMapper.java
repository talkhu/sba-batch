...
public interface PaymentBatchMapper {
	...
	@Select("SELECT id, courseId, cost/fee*100 as schedule,startDate, endDate, fee FROM payment.payment where cost/fee*100 != 100")
	List<BatchPayment> batchpayment();
	// update pahse
	@Update("update payment.payment set cost=fee*#{phase} where courseid = #{id}")
